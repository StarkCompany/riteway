...
ExcellentExport = (function() {
	...
	var ieBlobSave = function(ext, type, data) {
		try {
            var blob = new Blob([decodeURIComponent(encodeURI(data))], {
                type: type
            });
            var fileName = 'Report-' + new Date().getTime() + '.' + ext;
    ...
    var ee = {
        ...
        excel: function(anchor, table, name) {
            ...
            var excelData = format(template.excel, ctx);

			if (window.navigator.msSaveOrOpenBlob) {
			  ieBlobSave('xls', 'application/vnd.ms-excel', excelData);
			  return false;
			}

            var base64ExcelData = base64(excelData);
            var hrefvalue = uri.excel + base64ExcelData;
        ...
        csv: function(anchor, table, delimiter, newLine) {
			...
			if (window.navigator.msSaveOrOpenBlob) {
			  ieBlobSave('csv', 'text/csv;charset=utf-8;', csvData);
			  return false;
			}
